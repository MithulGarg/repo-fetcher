...
st.set_page_config(
    page_title="Trending GitHub Repositories", page_icon="🚀", layout="wide"
)

st.title("🚀 GitHub Trending Dashboard")
st.markdown(
    "Explore top trending GitHub repositories dynamically filtered by language and timeframe."
)
...
days = st.sidebar.slider("Timeframe (Days)", min_value=1, max_value=30, value=7)


@st.cache_data(ttl=3600)
def fetch_trending_github_repos(lang: str, days_ago: int) -> list[dict[str, Any]]:
    """Fetch repositories created within the selected timeframe and language."""
    date_threshold = (datetime.now(timezone.utc) - timedelta(days=days_ago)).strftime(
        "%Y-%m-%d"
    )
...
# Fetch data
with st.spinner(
    f"Fetching trending repositories for {language} over the last {days} days..."
):
...
if repositories:
    ...
    with col2:
        ...
        st.metric(
            "Top Repo",
            top_repo["Name"] if top_repo else "N/A",
            f"⭐ {top_repo['Stars']:,}" if top_repo else "",
        )
    with col3:
